...
@dataclass
class Controls:
    """Aircraft control surface deflections and throttle (4 independent surfaces)."""
    elevator: float = 0.0   # rad, virtual elevator (for compatibility)
    aileron:  float = 0.0   # rad, virtual aileron (for compatibility, positive = right wing down)
    rudder:   float = 0.0   # rad, virtual rudder (for compatibility)
    throttle: float = 0.0   # 0–1 (normalised)
    # Physical control surfaces (4 independent surfaces)
    aileron_left:  float = 0.0   # rad, left aileron deflection (positive = trailing-edge down)
    aileron_right: float = 0.0   # rad, right aileron deflection (positive = trailing-edge down)
    v_left:   float = 0.0   # rad, left V-tail deflection (positive = trailing-edge down)
    v_right:  float = 0.0   # rad, right V-tail deflection (positive = trailing-edge down)
...
class NonlinearModel:
    ...
    def state_dot(
        ...
        # For 4 independent surfaces configuration
        # Use physical surface deflections if available, otherwise convert from virtual
        if hasattr(controls, 'aileron_left') and hasattr(controls, 'aileron_right'):
            da_left = controls.aileron_left
            da_right = controls.aileron_right
        else:
            # Fallback: convert virtual aileron to physical surfaces
            # Positive aileron → right wing down (left aileron down, right aileron up)
            da_left = controls.aileron
            da_right = -controls.aileron
            
        dv_left = controls.v_left if hasattr(controls, 'v_left') else 0.0
        dv_right = controls.v_right if hasattr(controls, 'v_right') else 0.0
        throttle = controls.throttle

        # --- Aerodynamic forces & moments ------------------------------------
        aero = compute_aero_forces(
            u, v, w, pp, q, r, da_left, da_right, dv_left, dv_right, throttle, p,
    ...
    def simulate(
        ...
        def get_controls(t: float) -> Controls:
            ...
            # V-tail mixing: convert virtual elevator/rudder to physical V-tail
            dv_left  = de_cmd - dr_cmd
            dv_right = de_cmd + dr_cmd
            
            return Controls(
                elevator=0.0,  # Virtual (not used)
                aileron=da_cmd,
                rudder=0.0,    # Virtual (not used)
                throttle=thr,
                v_left=trim.de_trim + dv_left,
                v_right=trim.de_trim + dv_right,
            )

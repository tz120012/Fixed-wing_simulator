...
class ServoOutput:
    ...
    def to_radians(
        ...
        vtail_max_rad: float = np.radians(25.0),
    ):
        """
        Convert normalised outputs to surface deflections in radians.
        
        For 4 independent surfaces: left/right ailerons + left/right V-tails.

        Note: aerodynamics convention is positive = trailing-edge down.
        The control stack uses positive elevator = nose UP, so elevator
        sign is negated here at the interface.

        Returns
        -------
        (da_left, da_right, dv_left, dv_right, throttle) in radians
        
        Aileron mixing:
        - Positive aileron command → right wing down (left aileron down, right aileron up)
        - Each aileron contributes half of the total roll moment
        
        V-tail mixing:
        - Symmetric deflection (elevator) → both V-tails move together
        - Differential deflection (rudder) → V-tails move opposite
        """
        # Virtual control surfaces (from control stack)
        de_virtual = -self.elevator * elev_max_rad   # negate: ctrl +up → aero -trailing-edge
        dr_virtual =  self.rudder   * rud_max_rad
        
        # Aileron mixing: differential deflection for roll
        # Positive aileron command → right wing down
        # Left aileron: positive (trailing edge down) creates positive roll
        # Right aileron: negative (trailing edge up) creates positive roll
        da_left  =  self.aileron * ail_max_rad
        da_right = -self.aileron * ail_max_rad
        
        # V-tail mixing: convert virtual elevator/rudder to physical V-tail deflections
        # Left V-tail:  pitch - yaw
        # Right V-tail: pitch + yaw
        dv_left  = de_virtual - dr_virtual
        dv_right = de_virtual + dr_virtual
        
        # Saturate aileron deflections
        da_left  = saturate(da_left,  -ail_max_rad, ail_max_rad)
        da_right = saturate(da_right, -ail_max_rad, ail_max_rad)
        
        # Saturate V-tail deflections
        dv_left  = saturate(dv_left,  -vtail_max_rad, vtail_max_rad)
        dv_right = saturate(dv_right, -vtail_max_rad, vtail_max_rad)
        
        return da_left, da_right, dv_left, dv_right, self.throttle
